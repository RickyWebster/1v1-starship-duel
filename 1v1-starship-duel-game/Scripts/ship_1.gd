...
signal laser_shot1(laser, damaged)
		...
		if !shoot_cd:
			...
			if powered == true and passer.what_power1 != 1:
				if passer.what_power1 == 2:
					shoot_laser(10, damage)
					shoot_laser(0, damage)
					shoot_laser(-10, damage)
					count += 1
					if count >= 8:
						reset()
					...
					for i in range(8):
						if i != 4:
							shoot_laser(i * 45, damage)
					count += 1
					if count >= 4:
						reset()
				elif passer.what_power1 == 4:
					shoot_laser(0, 50)
					reset()
			else:
				shoot_laser(0, damage)
...
func shoot_laser(offset, damaged):
	if dead != true:
		...
		emit_signal("laser_shot1", las, damaged)
